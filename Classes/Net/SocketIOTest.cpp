...
SocketIOTestLayer::SocketIOTestLayer(void)
{
    ...
    auto labelSIOClient = LabelTTF::create("Send Query Msg", "Arial", 22);
	...
	// Sahred Status Label
    _sioClientStatus = LabelTTF::create("Not connected...", "Arial", 18, Size(320, 100), TextHAlignment::LEFT);
    _sioClientStatus->setAnchorPoint(Point(0, 0));
    _sioClientStatus->setPosition(Point(VisibleRect::left().x, VisibleRect::rightBottom().y + 100));
    ...
    addChild(menuBack);
	ProtocolThread::GetInstance()->registerMessageCallback(CC_CALLBACK_1(SocketIOTestLayer::testevent, this),this);
	//ResPonseThread::GetInstance()->registerMessageCallback(nullptr);
}
...
void SocketIOTestLayer::testevent(struct ProtobufCMessage* message) {
...
// 
 	//_sioClientStatus->setString(message->DebugString());
...
// 	login.email ="xijing@infolife.mobi";
//     login.passwd = "1111111";
// 	ProtocolThread::GetInstance()->sendMessage((struct ProtobufCMessage*)&login,PROTO_TYPE_EmailLogin);
