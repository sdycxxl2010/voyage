#ifndef _H_SDLOCALIZATION_
#define _H_SDLOCALIZATION_
#include "TVStaticDataBase.h"
class SDLocalization : public TVStaticDataBase
{
public:
	static const char* ConfigName()
	{
		return "SDLocalization";
	}
private:
	string ID;
	string text_en_US;
...
public:
	...
	static SDLocalization* GetElement(string& strElement)
	{
		TVStaticDataBase* base = TVStaticDataManager::getInstance()->GetElementByStrId(ConfigName(), strElement.c_str());
		return dynamic_cast<SDLocalization*>(base);
	}

	static SDLocalization* GetElement(int nElement)
	{
